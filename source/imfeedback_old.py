...
class ImageContainer:
    ...
    def generate_distorted_mesh(self, sigma=.5, clip=False):
        if clip==True:
            mesh_noise_x = np.clip(np.random.normal(scale=sigma, size=self.imshape), -0.5, 0.5)
            mesh_noise_y = np.clip(np.random.normal(scale=sigma, size=self.imshape), -0.5, 0.5)
        else:
    ...
    def run_feedback_loop_regen_mesh(self, max_iters=15, sigma=.2, clip=False):
        if max_iters == None:
            #ctrl+c to exit
            while True:
                self.generate_distorted_mesh(sigma=sigma, clip=clip)
                self.feedback()
                print 'iter'
        else:
            iter = 1
...
red_sunset = ImageContainer('/Users/Jonathan/Documents/python_programs/Red_sunset.png')

red_sunset.monet('/Users/Jonathan/Documents/python_programs/Red_sunset_monet.png')
